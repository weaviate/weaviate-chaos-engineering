    ...
    try:
        ...
        if not exists:
            if graphQLExists:
                ...
                raise Exception(
                    f"Mismatch for: {object_id} exists: head: {exists} graphql: {graphQLExists}"
                )
...
def existsGraphQLGet(object_id: str) -> bool:
    ...
    objectGraphQLGet = (
        client.query.get("ObjectsAreNotDeleted", ["_additional{id}"]).with_where(equalId).do()
    )
...
def search_object(object_id: str, should_exist: bool):
    ...
    if not (
        should_exist == headExists
        and should_exist == objectRestExists
        and should_exist == objectGraphQLExists
    ):
...
def delete_all_in_batch(uuids):
    try:
        operands = []
        for id in uuids:
            operands.append({"operator": "Equal", "path": ["id"], "valueString": id})
