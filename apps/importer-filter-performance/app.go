...
func do(ctx context.Context) error {
	...
	for count < size {
		...
		for i := 0; i < batchSize; i++ {
			strings := generateRandomlyOccurringStrings()
			stringsBytes, _ := json.Marshal(strings)
			batcher.addObject(fmt.Sprintf(`{"item_id":%d, "string_matches":%s}`, count+1, string(stringsBytes)), randomVector(dims))
		}
...
func generateRandomlyOccurringStrings() []string {
	out := []string{}

	for percentage := 1; percentage < 100; percentage += 1 {
		if rand.Intn(100) <= percentage {
			out = append(out, fmt.Sprintf("%d-percent-occurring", percentage))
			out = append(out, fmt.Sprintf("%d-percent-occurring-with-random-string-%d", percentage, rand.Intn(100)))
		}
	}

	out = append(out, "100-percent-occurring")
	out = append(out, fmt.Sprintf("100-percent-occurring-with-random-string-%d", rand.Intn(100)))
	return out
}
