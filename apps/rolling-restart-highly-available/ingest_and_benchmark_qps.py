...
def main() -> None:
    ...
    objects_tolerance_percentage = 1
    ...
    logger.info("Verifying object counts per collection...")
    ...
    for cfg in collections:
        ...
        try:
            ...
            if is_mt:
                tenants = client.collections.get(name).tenants.get().keys()
                if len(tenants) == 0:
                    logger.error(
                        "No tenants found for collection {name} when expecting to have one",
                        name=name,
                    )
                    failure_state["failed"] = True
                    continue
                tenant = list(tenants)[0]
                collection = client.collections.get(name).with_tenant(tenant)

            if (
                abs(len(collection) - objects_per_class)
                > objects_per_class * objects_tolerance_percentage / 100
            ):
                logger.error(
                    "Count mismatch for collection {name}: expected {exp}, got {got} (tolerance {tolerance}%)",
                    name=name,
                    exp=objects_per_class,
                    got=len(collection),
                    tolerance=objects_tolerance_percentage,
                )
                failure_state["failed"] = True
            else:
                logger.info(
                    "Count OK for {name}: {got}",
                    name=name,
                    got=len(collection),
                )

        except Exception as e:  # pragma: no cover
            logger.exception(f"Count verification failed for {name}: {e}")
            failure_state["failed"] = True
