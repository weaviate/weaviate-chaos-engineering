...
def _create_roles(client: weaviate.WeaviateClient):
    ...
    for role, permissions in defaults.EXPECTED_ROLES.items():
        client.roles.create(
            role_name=role,
            permissions=permissions,
        )
    expected_role_names = list(defaults.EXPECTED_ROLES.keys())
    sec, cutoff = 0, 120
    while sec < cutoff:
        sec = sec + 1
        roles = client.roles.list_all()
        role_names = list(roles.keys())
        if all(role in role_names for role in expected_role_names):
...
def _assign_roles(client: weaviate.WeaviateClient):
    ...
    roles = list(defaults.EXPECTED_ROLES.keys())
    ...
    while sec < cutoff:
        sec = sec + 1
        roles_assigned = client.users.get_assigned_roles(defaults.CUSTOM_USER).keys()
