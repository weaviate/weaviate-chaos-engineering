...
ssh -i "${key_id}.pem" $ssh_addr -- "ASYNC_INDEXING=$ASYNC_INDEXING DATASET=$dataset DISTANCE=$distance REQUIRED_RECALL=$REQUIRED_RECALL QUANTIZATION=$QUANTIZATION WEAVIATE_VERSION=$WEAVIATE_VERSION MACHINE_TYPE=$MACHINE_TYPE CLOUD_PROVIDER=$CLOUD_PROVIDER OS=$OS RQ_BITS=$RQ_BITS bash ann_benchmark_quantization.sh"
mkdir -p results
