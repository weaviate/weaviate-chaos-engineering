...
wait_for_hnsw_snapshot() {
    ...
    # In 1.39+, the old snapshot mechanism was replaced by compactv2 which emits different log messages.
    local version
    ...
    if [ "$version_major" -gt 1 ] || { [ "$version_major" -eq 1 ] && [ "$version_minor" -ge 38 ]; }; then
        ...
        echo "Weaviate < 1.39 detected (${version}), waiting for legacy snapshot"
    fi
