...
wait_for_condensing() {
    ...
    while [ $attempt -le $retries ]; do
        if docker compose -f apps/weaviate/docker-compose.yml logs weaviate | grep "starting forced compaction"; then
...
echo "Run updater script to increase the commit log size"
run_updater $SIZE

echo "Wait some time to let the metrics be updated"
sleep 30

echo "Restart the container to enforce condensing"
